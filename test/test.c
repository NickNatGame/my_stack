...
static void stack_destroy(my_stack *stack){
    free(stack->block);
    stack->block = NULL;
    stack->pointer = NULL;
}
static int stack_check_init(){
    ...
    SOFT_ASSERT_ERR((char*)stack.pointer == (char*)stack.block + (int)sizeof(CANARY)); /* |                                      */
    SOFT_ASSERT_ERR(check_canaries(&stack) == 1);                                      /* |   check pointer, canaries and hash   */
...
static int test_push_pop(void) {
    ...
    pop(&stack);                                            /* |                    */
    SOFT_ASSERT_EQ_ERR(stack.count_idx, -1);                /* |                    */
    SOFT_ASSERT_ERR((stack.error & STACK_UNDERFLOW) == 0);  /* |   check pop basic  */
    SOFT_ASSERT_ERR(stack.hash == hash_create(&stack));     /* |                    */
    SOFT_ASSERT_ERR(check_canaries(&stack) == 1);           /* |                    */
